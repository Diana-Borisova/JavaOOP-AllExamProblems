...
public class ControllerImpl implements Controller {
    ...
    public String finalResult() {
        ...
        build.append(String.format(FINAL_STATE_EXPLORED, exploredStates)).append("\n");
        build.append(FINAL_EXPLORER_INFO).append("\n");

        for (Explorer explorer : explorers.getCollection()) {

            build.append(String.format(FINAL_EXPLORER_NAME, explorer.getName())).append("\n");
            build.append(String.format(FINAL_EXPLORER_ENERGY , explorer.getEnergy())).append("\n");
            if (explorer.getSuitcase().getExhibits().isEmpty()) {
                build.append(String.format(FINAL_EXPLORER_SUITCASE_EXHIBITS , "None")).append("\n");
            } else {
                build.append(String.format(FINAL_EXPLORER_SUITCASE_EXHIBITS,
                        String.join(FINAL_EXPLORER_SUITCASE_EXHIBITS_DELIMITER,
                                explorer.getSuitcase().getExhibits()))).append("\n");
            }
        }
